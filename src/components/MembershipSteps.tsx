...
function MembershipSteps() {
  const [steps, setSteps] = useState<MembershipStep[]>([]);
  ...
  useEffect(() => {
    const fetchSteps = async () => {
      try {
        const q = query(collection(db, "membershipSteps"));
        const querySnapshot = await getDocs(q);
        const fetchedSteps = querySnapshot.docs
          .map((doc) => ({
            id: doc.id,
            ...doc.data(),
          } as MembershipStep))
        ...
        if (fetchedSteps.length > 0) {
          setActiveIndex(fetchedSteps[0].id);
        }
      } catch (error) {
        console.error("Error fetching membership steps: ", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="w-full flex items-center justify-center py-20">
        <div className="text-lg">Loading membership steps...</div>
      </div>
    );
  }
  ...
  return (
    <div className='w-full grid grid-cols-1 lg:grid-cols-[40%_60%]'>
      ...
      <div className='w-full'>
        ...
        <div className='my-5 px-5 lg:px-0 w-full lg:w-[80%]'>
          {steps.map((step, index) => {
            const isOpen = activeIndex === step.id;

            return (
              <div
                key={step.id}
                className="border-b border-default py-6 transition-all duration-300"
              >
                {/* Header / Trigger */}
                <button
                  onClick={() => setActiveIndex(isOpen ? null : step.id)}
                ...
                >
                  <div className="overflow-hidden">
                    <p className="text-gray-600 leading-relaxed pl-0 lg:pl-16">
                      {step.description}
                    </p>
                  </div>
                </div>
